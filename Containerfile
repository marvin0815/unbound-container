...
RUN apk add -U alpine-sdk flex bison openssl-dev expat-dev

RUN git clone -brelease-${UNBOUND_VERSION} https://github.com/NLnetLabs/unbound.git
WORKDIR /unbound
RUN ./configure --enable-shared=no --enable-static=no --prefix=/usr --sysconfdir=/etc --disable-sha1 --disable-dsa
RUN make -j$(nproc)
RUN make strip
RUN make install
